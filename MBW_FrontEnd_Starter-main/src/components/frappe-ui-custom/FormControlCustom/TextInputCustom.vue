...
import { computed, useSlots, useAttrs, ref, watch } from "vue";
...
const isEmpty = ref(
  props.isCheckEmpty && (!attrs?.value || attrs?.value === "")
);
...
const inputClasses = computed(() => {
  ...
  //   Thêm lớp 'border-red-500' nếu trường rỗng và isCheckEmpty được bật
  variantClasses += isEmpty.value ? " border-red-500" : "";
  [isEmpty.value];
...
let emitChange = (value: string) => {
  ...
  value === "" ? (isEmpty.value = true) : (isEmpty.value = false);
};
if (props.debounce) {
