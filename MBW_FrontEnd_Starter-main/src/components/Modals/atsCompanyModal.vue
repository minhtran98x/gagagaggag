...
async function callSetValue(values) {
	try {
		if (!_yyy.value.company_id) {
			createToast({
				title: __("Error"),
				text: __("ID công ty là bắt buộc."),
				icon: "x",
				iconClasses: "text-red-600",
			});
			return;
		}
		if (!_yyy.value.company_name) {
			createToast({
				title: __("Error"),
				text: __("tên công ty là bắt buộc."),
				icon: "x",
				iconClasses: "text-red-600",
			});
			return;
...
const dialogOptions = computed(() => {
	let title = !editMode.value ? "Thêm công ty" : "Sửa thông tin";
...
const sections = createResource({
	...
	cache: ["quickEntryFields", "ATS_Company"],
	params: { doctype: "ATS_Company", type: "Quick Entry" },
	auto: true,
});
