<template>
  <FadedScrollableDiv
    ...
    :class="[isLastSection ? '' : 'max-h-[320px]']"
  >
    <div
      ...
      <div class="flex items-center justify-between w-[65%]">
        ...
        >
            ...
            <NestedPopover>
              <template #target="{ open }">
                ...
                >
                  ...
                  <div
                    v-else
                    class="text-base leading-5 text-gray-500 truncate"
                  >
                    {{ field.placeholder }}
                  </div>
                  <template #suffix>
          ...
          <DateTimePicker
						...
						:disabled="Boolean(field.read_only)"
						@change="(data) => handleDateTimeChange(data, field.name)"
					/>
          <FormControl
            v-else-if="
          ...
          <StarRating
            ...
            @update:rating="
              (rating) => (
                (data[field.name] = rating), emit('update', field.name, rating)
              )
            "
          />
          <FormControl
...
import { DateTimePicker, Tooltip } from "frappe-ui";
...
// Theo dõi sự thay đổi của data và cập nhật các trường phụ thuộc
const _fields = ref([]);

watch(
  () => data, // Theo dõi props.fields
  (newFields) => {
    const all_fields = [];
...
function handleDateTimeChange(newValue, fieldName) {
	emit("update", fieldName, newValue)
}
</script>
